...
const PostPreview = ({ post }: { post: PostType }) => {
  ...
  return (
    <div
      ref={postPreview}
      className={`border-sec relative mb-16 mt-0 flex h-fit flex-col justify-center overflow-hidden opacity-0 drop-shadow-md md:mb-28 md:h-96 md:flex-row lg:drop-shadow-lg ${isPostPreviewInView ? "animate-fadeIn" : "translate-x-1/2"}`}
    >
      <img
        src={imageSmall}
        loading="lazy"
      ...
      <div className="prose flex h-full flex-col self-center rounded-b-lg bg-white/80 px-4 py-3 sm:w-5/6 md:w-3/5 md:rounded-r-badge md:rounded-bl-none md:p-8 lg:w-3/6">
        ...
        <div className={`flex items-center justify-between`}>
          ...
          <Link
            to={`/blog/${id}`}
            className={`cursor-custom-pointer ${windowHeight <= 320 && "text-sm"} w-fit select-none rounded-md bg-primary px-4 py-1 font-protest text-xl uppercase text-neutral no-underline duration-200 hover:bg-primary/90 hover:text-white md:px-8 md:py-2 md:text-2xl`}
            onClick={() => scrollToTop()}
          >
            Czytaj...
          </Link>
